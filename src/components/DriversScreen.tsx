...
export function DriversScreen() {
  ...
  // 🟢 FIXED: Image URL Logic (Points to Backend Endpoint)
  const getDriverImage = (driverId: string) => {
    // 1. Check if we have multiple images cached
    ...
    // 2. Default to the new backend endpoint
    // This endpoint handles the .png/.jpg check automatically
    return `${API_BASE}/driver_image/${driverId}`;
  };
      ...
      try {
        ...
        if (response.ok) {
          ...
          mappedDrivers.forEach(async (driver: Driver) => {
             // We skip this for now to rely on the reliable single endpoint first, 
             // but you can uncomment if your backend supports multi-image fetching
          });
  ...
  const getFallbackDrivers = (): Driver[] => {
    return [
      { id: "VER", name: "Max Verstappen", team: "Red Bull", nationality: "Dutch", number: "1", age: 28, f1_debut: 2015, world_champs: 4, race_starts: 233, wins: 71, podiums: 127, poles: 48, status: "Active", teamColor: teamColors['Red Bull'] },
      { id: "NOR", name: "Lando Norris", team: "McLaren", nationality: "British", number: "4", age: 26, f1_debut: 2019, world_champs: 1, race_starts: 152, wins: 11, podiums: 44, poles: 16, status: "Active", teamColor: teamColors['McLaren'] },
      { id: "LEC", name: "Charles Leclerc", team: "Ferrari", nationality: "Monégasque", number: "16", age: 28, f1_debut: 2018, world_champs: 0, race_starts: 171, wins: 8, podiums: 50, poles: 27, status: "Active", teamColor: teamColors['Ferrari'] }
    ];
  };
  ...
  const handleSendMessage = () => {
    ...
    setInputMessage('');
    setTimeout(() => {
      setChatMessages(prev => [...prev, { role: 'assistant', content: "I'm the Grand Prix Guru! Ask me about stats." }]);
    }, 1000);
  };
  ...
  return (
    <div className="min-h-screen pb-24 font-sans w-full transition-colors duration-300" style={containerStyle}>
      ...
      <div className="pt-32 px-4"> 
        ...
        <div className="mb-6">
          <button onClick={() => setUploadDialogOpen(true)} className={`w-full border-2 rounded-xl p-6 transition-all text-left group active:scale-[0.99] ${isDark ? 'bg-gradient-to-br from-red-900/20 to-neutral-900 border-red-900/50 hover:border-red-600' : 'bg-white border-white hover:border-red-200 shadow-md hover:shadow-lg'}`}>
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1"><h3 className={`font-bold mb-1 ${isDark ? 'text-white' : 'text-slate-900'}`}>Identify Driver AI</h3><p className={`text-sm ${isDark ? 'text-neutral-400' : 'text-slate-500'}`}>Upload a photo to see stats</p></div>
            </div>
          </button>
        </div>
        ...
        {/* Driver Grid */}
        {loading ? (
          <div className="text-center py-12"><div className="inline-block animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-red-600"></div><p className={`mt-2 ${isDark ? "text-neutral-500" : "text-slate-400"}`}>Loading drivers...</p></div>
        ) : (
          <>
            <div className="grid grid-cols-2 gap-3">
              {filteredDrivers.map((driver) => (
                <div key={driver.id} className={`rounded-xl border overflow-hidden transition-all relative group ${isDark ? 'bg-neutral-900/80 border-neutral-800 hover:border-neutral-700' : 'bg-white border-white shadow-sm hover:shadow-md hover:border-red-100'} ${driver.status !== 'Active' ? 'opacity-80' : ''}`}>
                  <div className={`relative aspect-square ${isDark ? 'bg-neutral-800' : 'bg-slate-100'}`}>
                    ...
                    <img 
                      ...
                      onError={(e) => {
                        // Retry with common extensions if first attempt fails
                        const target = e.currentTarget;
                        if (target.src.endsWith('.jpg')) target.src = target.src.replace('.jpg', '.png');
                        else target.src = 'https://via.placeholder.com/300x400/333333/666666?text=No+Image';
                      }}
                    />
                  ...
                  <div className="p-3">
                    ...
                    <div className="flex gap-3 mt-2 text-[10px]">
                      ...
                      <span className={isDark ? 'text-neutral-400' : 'text-slate-500'}>🥇 {driver.podiums}</span>
                    </div>
                  </div>
                </div>
              ))}
            </div>
            {filteredDrivers.length === 0 && <div className="text-center py-12"><p className={isDark ? "text-neutral-500" : "text-slate-400"}>No drivers found</p></div>}
          </>
        )}
      </div>
      ...
      <Dialog open={uploadDialogOpen} onOpenChange={setUploadDialogOpen}>
        <DialogContent className={`max-w-[90vw] max-h-[90vh] overflow-y-auto rounded-2xl border shadow-2xl transition-colors duration-300 z-50 ${isDark ? '!bg-neutral-900 border-neutral-800 text-white' : '!bg-white border-slate-200 text-slate-900'}`} style={{ backgroundColor: isDark ? '#171717' : '#ffffff' }}>
          <DialogHeader>
            <DialogTitle className={isDark ? 'text-white' : 'text-slate-900'}>Identify Driver AI</DialogTitle>
            <DialogDescription className={isDark ? 'text-neutral-400' : 'text-slate-500'}>Upload a photo to identify the driver.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            {!uploadedImage ? (
              <label className={`flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-xl cursor-pointer transition-colors ${isDark ? 'border-neutral-700 bg-neutral-950 hover:border-red-600' : 'border-slate-300 bg-slate-50 hover:border-red-400 hover:bg-slate-100'}`}>
                <div className="flex flex-col items-center justify-center pt-5 pb-6"><Upload className={`w-10 h-10 mb-3 ${isDark ? 'text-neutral-500' : 'text-slate-400'}`} /><p className={`mb-2 text-sm ${isDark ? 'text-neutral-400' : 'text-slate-600'}`}>Tap to upload</p></div>
                <input type="file" className="hidden" accept="image/*" onChange={handleFileUpload} />
              </label>
            ) : (
              <div className="space-y-4">
                <div className="relative w-full h-72">
                  <img src={uploadedImage} alt="Uploaded" className="w-full h-full object-cover object-top rounded-xl shadow-md" />
                  <button onClick={() => { setUploadedImage(null); setSelectedDriver(null); }} className="absolute top-2 right-2 z-10 w-9 h-9 bg-black/70 rounded-full flex items-center justify-center text-white hover:bg-black/90 backdrop-blur-md border border-white/30 shadow-xl transition-transform active:scale-90"><X className="w-5 h-5" /></button>
                </div>
                {selectedDriver && (
                  <div className={`rounded-xl p-4 border transition-colors shadow-lg mt-2 ${isDark ? 'bg-neutral-950 border-neutral-800' : 'bg-slate-50 border-slate-200'}`}>
                    <div className="flex items-center gap-4 mb-4">
                      <div className="w-2 h-12 rounded-full shadow-sm flex-shrink-0" style={{ backgroundColor: selectedDriver.teamColor }} />
                      <div className="flex-1">
                        <div className="flex justify-between items-start">
                          <div>
                            <h3 className={`font-black text-lg italic tracking-tight leading-none ${isDark ? 'text-white' : 'text-slate-900'}`}>{selectedDriver.name.toUpperCase()}</h3>
                            <p className={`text-xs mt-1 ${isDark ? 'text-neutral-400' : 'text-slate-500'}`}>{selectedDriver.team} • {selectedDriver.nationality}</p>
                          </div>
                          {selectedDriver.status !== 'Active' && <span className="bg-gray-800 text-white text-[10px] px-2 py-1 rounded-full">RETIRED</span>}
                        </div>
                      </div>
                    </div>
                    {/* Stats Grid */}
                    <div className={`grid grid-cols-4 gap-2 pt-3 border-t mb-4 ${isDark ? 'border-neutral-800' : 'border-slate-200'}`}>
                        <div className="text-center"><div className={`font-bold text-lg leading-none ${isDark ? 'text-white' : 'text-slate-900'}`}>{selectedDriver.wins}</div><div className={`text-[9px] uppercase mt-1 ${isDark ? 'text-neutral-500' : 'text-slate-400'}`}>Wins</div></div>
                        <div className="text-center"><div className={`font-bold text-lg leading-none ${isDark ? 'text-white' : 'text-slate-900'}`}>{selectedDriver.podiums}</div><div className={`text-[9px] uppercase mt-1 ${isDark ? 'text-neutral-500' : 'text-slate-400'}`}>Podiums</div></div>
                        <div className="text-center"><div className={`font-bold text-lg leading-none ${isDark ? 'text-white' : 'text-slate-900'}`}>{selectedDriver.poles}</div><div className={`text-[9px] uppercase mt-1 ${isDark ? 'text-neutral-500' : 'text-slate-400'}`}>Poles</div></div>
                        <div className="text-center"><div className={`font-bold text-lg leading-none ${isDark ? 'text-white' : 'text-slate-900'}`}>{selectedDriver.world_champs}</div><div className={`text-[9px] uppercase mt-1 ${isDark ? 'text-neutral-500' : 'text-slate-400'}`}>Titles</div></div>
                    </div>
                    <Button onClick={() => { setSearchQuery(selectedDriver.name); setUploadDialogOpen(false); setUploadedImage(null); setSelectedDriver(null); }} className="w-full bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl shadow-md">View Full Profile</Button>
                  </div>
                )}
              </div>
            )}
        ...
        <SheetContent side="bottom" className="bg-neutral-900/95 border-t-2 border-red-600 text-white h-[85vh] rounded-t-3xl p-0">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
            {chatMessages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[80%] rounded-2xl px-4 py-3 ${msg.role === 'user' ? 'bg-red-600' : 'bg-neutral-800'}`}><p className="text-sm">{msg.content}</p></div></div>
            ))}
          </div>
          <div className="p-4 bg-neutral-900">
            <div className="flex gap-2">
              <Input value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} className="bg-neutral-800 border-neutral-700 text-white" placeholder="Ask AI..." onKeyPress={(e) => e.key === 'Enter' && handleSendMessage()} />
